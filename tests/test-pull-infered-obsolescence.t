...
Create the three repositories we needs
  ...
  $ sed "s,$TESTTMP/base-case/,../," ./hg-repo/.hg/hgrc > ./hg-repo/.hg/hgrc.new
  $ mv ./hg-repo/.hg/hgrc.new ./hg-repo/.hg/hgrc
  $ hg -R hg-repo log -G
...
Pull replacement from a custom path using the default value
-----------------------------------------------------------

The config value for the `*` PATH should be used if no other config is set.

  $ count=50
  $ cp -r base-case test-config-custom-default-config
  $ cd test-config-custom-default-config
  ...
  $ fn_hg_commit -m gamma
  $ hg push -B some-branch
  pushing to $TESTTMP/test-config-custom-default-config/server.git
  searching for changes
  adding objects
  added 3 commits with 3 trees and 3 blobs
  adding reference refs/heads/some-branch
...
The default config value should be picked and the pull should create markers.

  $ cat << EOF >> ./hg-repo/.hg/hgrc
  > [paths]
  > default = NOTHING/TO/SEE/HERE
  > my-path = ../server.git
  > *:hg-git.find-successors-in=master
  > EOF
  $ hg -R hg-repo up master
  0 files updated, 0 files merged, 3 files removed, 0 files unresolved
  (activating bookmark master)
  $ hg -R hg-repo pull my-path
  pulling from $TESTTMP/test-config-custom-default-config/server.git
  importing 4 git commits
  HG-GIT:INFER_OBS: e91937fd8904 -> 05f94c07c58c
  HG-GIT:INFER_OBS: 67ec0a3230ae -> 47a01f7be6e7
  HG-GIT:INFER_OBS: 0c8e47eb927a -> 45f6ceb621f2
